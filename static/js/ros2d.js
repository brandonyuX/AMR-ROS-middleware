 ...
 *   * continuous (optional) - if the map should be continuously loaded (e.g., for SLAM)
 */
ROS2D.OccupancyGridClientNYS = function(options) {
  var that = this;
  options = options || {};
  var ros = options.ros;
  var topic = options.topic || '/map';
  ...
  poseListener.subscribe(function(pose) {
    ...
    if (!initScaleSet) {
      ...
      initScaleSet = true;
		...
		//shapeX.rotation = stage.rosQuaternionToGlobalTheta(pose.orientation);
    // change the angle
    robotMarker.rotation = stage.rosQuaternionToGlobalTheta(pose.orientation);
		//poseDir = stage.rosQuaternionToGlobalTheta(pose.orientation);
    robotMarker.visible = true;
		//stage.update();
  });
};
ROS2D.OccupancyGridClientNYS.prototype.__proto__ = EventEmitter2.prototype;
...
ROS2D.NavigationArrow = function(options) {
  ...
  // draw the arrow  : globotix
  var graphics = new createjs.Graphics();
  // line width
  graphics.setStrokeStyle(strokeSize);	
  //graphics.moveTo(0,0);
  graphics.beginStroke(strokeColor);
	graphics.beginFill("#c2c0d7");
  ...
  graphics.lineTo(size/2.0, size/2.0);
	...
	graphics.moveTo(-3,(size/2.0)-3); 	
	graphics.drawEllipse (-3,(size/2.0)-3,6,3);
	graphics.moveTo(-3,(-size/2.0)); 	
	graphics.drawEllipse (-3,(-size/2.0),6,3);
	graphics.moveTo(size/2.0,0); 
	graphics.drawCircle (size/2.0,0,3);
	//graphics.moveTo(50,50); 
	//graphics.drawCircle (50,50,5);  
	graphics.endFill();
  graphics.closePath();  
  graphics.endStroke();
...
ROS2D.Viewer = function(options) {
  ...
  var canvas = document.createElement('canvas');
  canvas.width = window.innerWidth/2-10; //600;//window.innerWidth; //this.width;
	this.width = window.innerWidth/2-10; //600;//window.innerWidth;
  canvas.height = 400;//window.innerHeight; //this.height;
